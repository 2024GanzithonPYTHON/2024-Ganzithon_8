...
const AiMessage = () => {
  ...
  useEffect(() => {
    const fetchAiMessageAndDiaryImage = async () => {
      ...
      try {
        ...
        const aiResponse = await fetch(`${process.env.REACT_APP_API_URL}/gpt/message`);
        const aiData = await aiResponse.json();
        setMessage(aiData.message || "AI 메시지를 가져올 수 없습니다.");
        ...
        if (diaryResponse.ok) {
          ...
          const diaryImage = diaryData.data.diaries[0]?.diaryImage ||
  ...
  const handleConfirmClick = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/save-positive-message`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
